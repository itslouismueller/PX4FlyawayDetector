...
extern "C" __EXPORT int crosstrack_err_main(int argc, char *argv[]);

int crosstrack_err_main(int argc, char *argv[])
{
    ...
    while(true) {
        ...
        if (poll_ret == 0) {
            ...
            if (fds[0].revents & POLLIN) {
                ...
                if (movingAvg < 0){
                    PX4_INFO("PROGRESS ---- WP Dist (m): %8.4f  MA Progress Rate (m/s): %8.4f  Inst Rate (m/s): %8.4f  Rolling Tot: %8.4f", current_dist_m, movingAvg,  buffer[i%windowSize], rollingTot);
                }
                else{
                    PX4_INFO("NO PROGRESS - WP Dist (m): %8.4f  MA Progress Rate (m/s): %8.4f  Inst Rate (m/s): %8.4f  Rolling Tot: %8.4f", current_dist_m, movingAvg,  buffer[i%windowSize], rollingTot);
                }
